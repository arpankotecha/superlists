...
REPO_URL = 'https://github.com/arpankotecha/superlists.git'
...
def _update_virtualenv(source_folder):
  ...
  if not exists(virtualenv_folder + '/bin/pip'):
    run('virtualenv --python=python3 {}'.format(
        virtualenv_folder))
...
def _update_static_files(source_folder):
  run("cd {} && ../virtualenv/bin/python3 "
      "manage.py collectstatic --noinput".format(
          source_folder))
# end

def _update_database(source_folder):
  run("cd {} && ../virtualenv/bin/python3 "
      "manage.py migrate --noinput".format(
          source_folder))

def deploy():
  site_folder = '/home/{}/sites/{}'.format(env.user, 
      env.host)
